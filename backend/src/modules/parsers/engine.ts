...
import { sanitizeTelemetryPayload } from '../ingestion/sanitizer';
...
export class UniversalLogParserEngine {
  private parsers: LogParserRule[] = [
    {
  ...
  public getParsers(): LogParserRule[] {
    return this.parsers;
  }

  public parseRawLog(rawLog: string, parserId?: string): ParseResult {
    try {
  ...
  public async ingestParsedLog(rawLog: string, parserId?: string) {
    ...
    const redactedMetadata = sanitizeTelemetryPayload(norm.metadata);
    ...
    await eventBus.publishTelemetry({
      id: event.id,
...
export const logParserEngine = new UniversalLogParserEngine();
