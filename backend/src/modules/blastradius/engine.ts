...
export interface BlastRadiusNode {
  id: string;
  label: string;
  type: 'identity' | 'iam_role' | 'vpc' | 'database' | 's3_bucket' | 'api_key' | 'token' | 'compute';
...
export interface BlastRadiusResult {
  ...
  blastSeverityIndex: number; // 0-100
  estimatedExposureUsd: number;
  totalReachableAssets: number;
  nodes: BlastRadiusNode[];
  edges: BlastRadiusEdge[];
  containmentActions: { action: string; priority: 'IMMEDIATE' | 'HIGH' | 'STANDARD' }[];
}

// Simulated enterprise infrastructure graph
const INFRASTRUCTURE_GRAPH: Record<string, { nodes: BlastRadiusNode[]; edges: BlastRadiusEdge[] }> = {
  '198.51.100.77': {
    nodes: [
...
export class BlastRadiusEngine {
  public async analyzeEntity(entityKey: string): Promise<BlastRadiusResult> {
    ...
    const containmentActions = [
      { action: `Enforce Edge WAF IP block on ${cleanKey} across all ingress zones`, priority: 'IMMEDIATE' as const },
      { action: `Revoke all IAM session tokens and rotate compromised access keys`, priority: 'IMMEDIATE' as const },
      { action: `Isolate affected VPC subnets and disable cross-region peering`, priority: 'HIGH' as const },
      { action: `Enable S3 Object Lock (WORM) on PII buckets to prevent deletion`, priority: 'HIGH' as const },
      { action: `Force password reset for all identities authenticated from ${cleanKey}`, priority: 'HIGH' as const },
      { action: `Snapshot and preserve forensic disk images of affected EC2 instances`, priority: 'STANDARD' as const },
    ];
  ...
  public async lockPerimeter(entityKey: string): Promise<{ success: boolean; message: string; actionsExecuted: string[] }> {
    const cleanKey = entityKey.replace('ip:', '').replace('user:', '');
