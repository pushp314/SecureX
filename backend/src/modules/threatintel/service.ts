import { threatIntelEngine } from './engine';

export interface ThreatIntelResult {
  isMalicious: boolean;
  threatScore: number; // 0 - 100
  threatType?: string; // e.g. 'APT_INFRASTRUCTURE', 'TOR_EXIT_NODE', 'BOTNET_C2', 'KNOWN_SCANNER'
  threatActor?: string; // e.g. 'APT29 (Cozy Bear)', 'Lazarus Group', 'Anonymous'
  country?: string;
  asn?: string;
  tags: string[];
...
const KNOWN_IOC_DATABASE: Record<string, ThreatIntelResult> = {
  '198.51.100.77': {
    isMalicious: true,
    threatScore: 94,
...
export function lookupThreatIntelligence(ipOrIdentifier: string): ThreatIntelResult {
  // Check dynamic engine indicators first
  const engineCheck = threatIntelEngine.checkIndicator(ipOrIdentifier);
...
export function enrichTelemetryWithThreatIntel(event: any): any {
