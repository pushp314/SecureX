...
const IDENTITIES_STORE: ManagedIdentity[] = [
  {
    id: 'id-01',
...
export function getManagedIdentities(): ManagedIdentity[] {
  return IDENTITIES_STORE;
}

export function containIdentity(id: string, action: 'LOCK' | 'REVOKE_SESSIONS' | 'REQUIRE_MFA'): ManagedIdentity {
...
export async function runIdentityStressTest(scenario: 'MFA_FATIGUE' | 'GOLDEN_SAML' | 'KERBEROASTING' | 'STALE_WORKLOAD', targetUsername?: string): Promise<IdentityStressTestResult> {
