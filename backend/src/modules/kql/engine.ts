...
export class SiemQueryLanguageEngine {
  public async executeQuery(kqlQuery: string): Promise<KqlQueryResult> {
    ...
    let dataset: any[] = rawEvents.map((evt) => ({
      id: evt.id,
      eventId: evt.eventId,
  ...
  private applyFilter(dataset: any[], filterStr: string): any[] {
  ...
  private applyStatsAggregation(dataset: any[], statsExpr: string): any[] {
    ...
    dataset.forEach((row) => {
      ...
      if (!groups[key]) {
        const initialSample: any = { count: 0 };
  ...
  private applySort(dataset: any[], sortExpr: string): any[] {
  ...
  private generateHistogram(rawEvents: any[]): { timeBucket: string; count: number }[] {
  ...
  private extractDiscoveredFields(rawEvents: any[]) {
    const fields = [
      'category',
      'eventType',
      'severity',
    ...
    return fields.map((f) => {
      const values = Array.from(new Set(rawEvents.map((r: any) => r[f]).filter(Boolean))) as string[];
      return {
        name: f,
        type: 'string',
...
export const siemQueryEngine = new SiemQueryLanguageEngine();
