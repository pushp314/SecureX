export interface UebaEntityProfile {
  ...
  baseline: {
    ...
    normalWorkingHours: string; // e.g. "09:00 - 18:00 UTC"
  ...
  zScore: {
    ...
    egressZScore: number;
    timeZScore: number;
    compositeAnomalyScore: number; // 0 - 100
  };
  anomalyFlag: boolean;
...
export class UebaAnalyticsEngine {
  private profiles: UebaEntityProfile[] = [
    {
      id: 'UEBA-USR-01',
  ...
  public getProfiles(): UebaEntityProfile[] {
    return this.profiles;
  }

  public getAnomalousProfiles(): UebaEntityProfile[] {
    return this.profiles.filter((p) => p.anomalyFlag);
  }
}

export const uebaEngine = new UebaAnalyticsEngine();
