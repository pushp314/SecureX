...
const PAST_ANALYSES: MalwareSandboxAnalysis[] = [];

export function analyzeMalwareSample(sampleName: string, fileHash?: string): MalwareSandboxAnalysis {
  const hash = fileHash || crypto.createHash('sha256').update(sampleName + Date.now()).digest('hex');
  ...
  let verdict: 'MALICIOUS' | 'SUSPICIOUS' | 'CLEAN' = 'MALICIOUS';
  let threatScore = 96;
  ...
  const interceptedApiCalls = [
    { apiName: 'VirtualAllocEx', dll: 'KERNEL32.dll', category: 'Memory Allocation', arguments: 'AllocationType=MEM_COMMIT|MEM_RESERVE, Protect=PAGE_EXECUTE_READWRITE', threatWeight: 'CRITICAL' as const },
    { apiName: 'WriteProcessMemory', dll: 'KERNEL32.dll', category: 'Process Injection', arguments: 'TargetPid=5124, Bytes=65536', threatWeight: 'CRITICAL' as const },
    { apiName: 'CreateRemoteThread', dll: 'KERNEL32.dll', category: 'Execution', arguments: 'ThreadRoutine=0x7ffb12340000', threatWeight: 'CRITICAL' as const },
    { apiName: 'RegSetValueExA', dll: 'ADVAPI32.dll', category: 'Persistence', arguments: 'Key=HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run', threatWeight: 'HIGH' as const },
    { apiName: 'InternetOpenA', dll: 'WININET.dll', category: 'C2 Communication', arguments: 'UserAgent=Mozilla/5.0 (Windows NT 10.0)', threatWeight: 'MEDIUM' as const },
  ];
  ...
  const analysis: MalwareSandboxAnalysis = {
...
export function getPastSandboxAnalyses(): MalwareSandboxAnalysis[] {
