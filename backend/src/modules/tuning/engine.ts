...
export interface TuningAction {
  ...
  newThreshold: number;
  suppressionFilters: string[];
  appliedAt: string;
  estimatedNoiseReduction: number; // percentage
}

// Simulated detection rule performance metrics
const RULE_METRICS: Record<string, { firings: number; escalated: number; dismissed: number; threshold: number }> = {
...
export class AdaptiveTuningEngine {
  ...
  public applyTuning(ruleId: string): TuningAction {
    const metrics = RULE_METRICS[ruleId];
    if (!metrics) throw new Error(`Rule not found: ${ruleId}`);

    const fpr = Math.round((metrics.dismissed / (metrics.firings || 1)) * 100);
    const previousThreshold = metrics.threshold;
    let newThreshold = previousThreshold;
    const suppressionFilters: string[] = [];
...
export const tuningEngine = new AdaptiveTuningEngine();
