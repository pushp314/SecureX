...
exports.HUNT_TEMPLATES = void 0;
exports.executeThreatHunt = executeThreatHunt;
exports.convertHuntToDetectionRule = convertHuntToDetectionRule;
const client_1 = require("../../db/client");
exports.HUNT_TEMPLATES = [
    {
        id: 'hunt-01',
...
async function executeThreatHunt(filter) {
    ...
    if (filter.outcome)
        whereClause.outcome = filter.outcome;
    if (filter.severity)
    ...
    for (const e of events) {
        if (e.sourceIp)
            entities.add(`ip:${e.sourceIp}`);
        if (e.username)
            entities.add(`user:${e.username}`);
    }
    return {
...
async function convertHuntToDetectionRule(data) {
    const ruleId = `SECX-R-HUNT-${Date.now().toString().slice(-4)}`;
    const createdRule = await client_1.prisma.detectionRule.create({
        data: {
            ruleId,
            name: data.name,
