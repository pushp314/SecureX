...
exports.getDeadLetterEvents = getDeadLetterEvents;
exports.captureDeadLetterEvent = captureDeadLetterEvent;
exports.reprocessDeadLetterEvent = reprocessDeadLetterEvent;
...
function getDeadLetterEvents() {
    return DLQ_STORE;
}
function captureDeadLetterEvent(errorReason, rawPayload, sourceIp = '127.0.0.1', sourceService = 'ingestion-gateway') {
    const dlqItem = {
        id: `dlq-${Date.now()}`,
...
async function reprocessDeadLetterEvent(id, repairedCategory = 'threat_signal') {
    ...
    await eventBus_1.eventBus.publishTelemetry(repairedEvent);
    item.status = 'REPROCESSED';
    item.retryCount += 1;
    return item;
}
function purgeDeadLetterQueue() {
    DLQ_STORE.length = 0;
