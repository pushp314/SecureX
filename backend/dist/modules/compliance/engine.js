...
exports.getComplianceFrameworks = getComplianceFrameworks;
exports.getD3FENDMatrix = getD3FENDMatrix;
...
async function getComplianceFrameworks() {
    const openIncidents = await client_1.prisma.incident.count({
        where: { status: 'OPEN' },
    });
    const criticalIncidents = await client_1.prisma.incident.count({
        ...
        return {
            ...
            controls: f.controls.map((c) => {
                if (c.controlCode === 'CC7.3' && criticalIncidents > 0) {
                    return { ...c, status: 'WARNING', evidencedBy: `Active alerts (${criticalIncidents} Critical Incident Cases)` };
                }
                return c;
            }),
        };
...
function getD3FENDMatrix() {
    return D3FEND_MATRIX;
}
async function generateExecutiveComplianceReport() {
